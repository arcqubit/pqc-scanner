...
use pqc_scanner::{
    analyze, export_itsg33_json, export_unified_json, generate_itsg33_report,
    generate_unified_report, SecurityClassification,
};
...
"#;
    ...
    println!("  Total vulnerabilities: {}", audit_result.stats.total_vulnerabilities);
    ...
    println!("  Prohibited: {:?}", itsg33_report.summary.cccs_prohibited_algorithms);
    println!("  Deprecated: {:?}", itsg33_report.summary.cccs_deprecated_algorithms);
    println!(
        "  Quantum-Vulnerable: {:?}",
    ...
    println!("  Validated: {}", itsg33_report.summary.cmvp_validated_count);
    println!();

    println!("Findings ({}):", itsg33_report.findings.len());
    for finding in &itsg33_report.findings {
        println!("  - {} ({:?})", finding.finding_id, finding.cccs_approval_status);
        println!("    ITSP References: {}", finding.itsp_references.join(", "));
        println!(
    ...
    println!(
        "Classification: {}",
        protected_b_report.control_assessment.security_classification
    );
    println!(
        "Compliance Score: {}/100",
    ...
    println!("  Control: {}", unified_report.nist_sc13_assessment.control_id);
    println!(
        "  Compliance Score: {}/100",
    ...
    println!("  Control: {}", unified_report.itsg33_sc13_assessment.control_id);
    println!(
        "  Classification: {}",
        unified_report.itsg33_sc13_assessment.security_classification
    );
    println!(
        "  Compliance Score: {}/100",
    ...
    for mapping in &unified_report.control_mapping {
        println!("  {} ↔ {}", mapping.nist_control_id, mapping.itsg33_control_id);
    ...
    println!("Unified Recommendations ({}):", unified_report.recommendations.len());
    for (i, rec) in unified_report.recommendations.iter().enumerate().take(5) {
        println!("  {}. {}", i + 1, rec);
    }
    if unified_report.recommendations.len() > 5 {
        println!("  ... and {} more", unified_report.recommendations.len() - 5);
    }
    println!();
